                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    { // k nearest neighbors (k=2)
        ...
        for (std::vector<std::vector<cv::DMatch>>::iterator it = matchesKnn.begin(); it<matchesKnn.end(); it++)
        {
            auto first = it->begin();
            auto second = first+1;
            if( (first->distance / second->distance) < 0.8)
                matches.insert(matches.end(),*(it->begin()));
        }
    }
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    {
        extractor = cv::xfeatures2d::SIFT::create(500);
    }
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    int minResponse = 52; // minimum value for a corner in the 8bit scaled response matrix
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "Harris detector extraction in " << 1000 * t / 1.0 << " ms" << endl;
...
bool detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
//    FAST, BRISK, ORB, AKAZE, SIFT
     ...
     if(detectorType.compare("FAST") == 0)
     {
         ...
         cv::FAST(img, keypoints, minThreshold, nonmaxSupression);
     }
     ...
     t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
     cout << detectorType << " detector finished in " << 1000 * t / 1.0 << " ms" << endl;
